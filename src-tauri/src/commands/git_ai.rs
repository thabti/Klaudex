...
//!
//! Spawns the user's configured agent CLI (`claude chat --no-interactive`)
...
//! * No new credential surface — reuses whatever auth `claude` already has.
...
pub async fn git_generate_commit_message(
    ...
    let (claude_bin, current_branch, diff_text, custom_instructions) = {
        ...
        (claude_bin, branch, diff, instructions)
    };
...
pub(crate) async fn run_claude_oneshot(claude_bin: &str, cwd: &str, prompt: &str) -> Result<String, AppError> {
    ...
    let child = Command::new(claude_bin)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| AppError::Other(format!("Failed to spawn '{claude_bin}': {e}")))?;
...
/// Strip claude decoration and extract the JSON `{ subject, body }` payload.
///
/// `claude chat --no-interactive` emits something like:
///
/// ```text
...
/// Bypasses Tauri state by accepting claude_bin and cwd directly.
pub async fn generate_for_smoke(claude_bin: &str, cwd: &str) -> Result<GeneratedCommitMessage, AppError> {
...
mod tests {
    ...
    fn parse_commit_response_errors_when_no_json() {
        let raw = "claude error: not authenticated\n";
    ...
    /// End-to-end smoke test against a real `claude` install.
    ///
    /// Ignored by default — run explicitly with:
    ///
    /// ```sh
    /// cargo test --manifest-path klaudex/src-tauri/Cargo.toml \
    ///   --lib commands::git_ai::tests::end_to_end_against_real_claude \
    ///   -- --ignored --nocapture
    /// ```
    #[tokio::test]
    #[ignore = "requires a working claude on PATH and network"]
    async fn end_to_end_against_real_claude() {
        ...
        let raw = run_claude_oneshot("claude", path.to_str().unwrap(), &prompt)
            .await
            .expect("claude oneshot failed");
