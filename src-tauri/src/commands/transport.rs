...
//! - **Stdio**: Communicates with a subprocess via stdin/stdout (current klaudex behavior)
//! - **HTTP (SSE)**: Communicates with remote MCP servers via HTTP + Server-Sent Events
//!
//! This allows klaudex to support both local CLI-based MCP servers and remote
